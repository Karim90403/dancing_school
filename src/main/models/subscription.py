...
class Subscription(IdMixin):
    ...
    lessons_left = models.SmallIntegerField("Прошло занятий", validators=[MinValueValidator(0)], null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        for el in Subscription.objects.all().filter(client=self.client):
            if el.status == "ACTIVE" and el.end_date > self.start_date > el.start_date and el != self:
                return "exist active"
            elif (
                (el.end_date > self.start_date > el.start_date) or (el.start_date < self.end_date < el.end_date)
            ) and el != self:
                return "time"
