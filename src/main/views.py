...
def groups_schedule(request):
    ...
    try:
        ...
        return render(request, "schedule.html",
                      {"data": Schedule.objects.filter(class_date=search_date).order_by('group')})
...
def test_classes(request):
    ...
    try:
        search_date = datetime.datetime.strptime(search_query, '%d-%m-%Y').date()
        return render(request, "test_class.html", {"data": TestClass.objects.annotate(
            number_of_classes=Count('clientrecord')).filter(class_date=search_date).order_by('choreographer')})
    ...
    for choreographer_id in set(choreographer_ids):
        for schedule in TestClass.objects.annotate(number_of_classes=Count('clientrecord')).filter(
                choreographer=choreographer_id, class_date__gt=datetime.date.today()).order_by('choreographer'):
            classes.append(schedule)
    if not search_query:
        return render(request, "test_class.html", {
            "data": TestClass.objects.annotate(number_of_classes=Count('clientrecord')).filter(
                class_date__gt=datetime.date.today()).order_by('choreographer')})
